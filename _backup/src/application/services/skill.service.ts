...
import { MOCK_SKILLS, filterAndPaginateSkills } from '@/data';
...
export class SkillService {
  ...
  async getSkills(filters: SkillFilters): Promise<SkillListResult> {
    try {
      if (process.env.MOCK_MODE === 'true') {
        const { data, total, pages } = filterAndPaginateSkills(MOCK_SKILLS, filters);
        return {
          data,
          pagination: {
            page: filters.page,
  ...
  async createSkill(skillData: Partial<Skill>): Promise<Skill> {
    try {
      if (process.env.MOCK_MODE === 'true') {
        ...
        // push into mock array for session scope only
        (MOCK_SKILLS as Skill[]).push(created);
        return created;
      }
  ...
  async updateSkill(id: string, updateData: Partial<Skill>): Promise<Skill | null> {
    try {
      if (process.env.MOCK_MODE === 'true') {
        ...
        const updated = Object.assign(MOCK_SKILLS[idx], updateData, { updated_at: new Date() });
        return updated;
      }
  ...
  async deleteSkill(id: string): Promise<boolean> {
    try {
      if (process.env.MOCK_MODE === 'true') {
        const before = MOCK_SKILLS.length;
        const remaining = MOCK_SKILLS.filter((s) => s.id !== id);
        (MOCK_SKILLS as Skill[]).length = 0;
        (MOCK_SKILLS as Skill[]).push(...remaining);
  ...
  async getSkillsByCompetency(competencyId: string): Promise<Skill[]> {
    try {
      if (process.env.MOCK_MODE === 'true') {
        // In mock mode, return all for simplicity
        return MOCK_SKILLS;
      }
