...
import { MOCK_COMPETENCIES, filterAndPaginateCompetencies } from '@/data';
...
export class CompetencyService {
  ...
  async getCompetencies(filters: CompetencyFilters): Promise<CompetencyListResult> {
    try {
      if (process.env.MOCK_MODE === 'true') {
        const { data, total, pages } = filterAndPaginateCompetencies(MOCK_COMPETENCIES, filters);
        return {
          data,
          pagination: {
            page: filters.page,
  ...
  async getCompetencyById(id: string): Promise<Competency | null> {
    try {
      if (process.env.MOCK_MODE === 'true') {
        const found = MOCK_COMPETENCIES.find((c) => c.id === id);
        return found || null;
      }
